...
type Decoder = (tp: number, count: number, size: number, sp: number, length: number) => number
type Filter = (tp: number, count4: number, size: number) => void

interface DecoderInstance extends WebAssembly.Instance {
  exports: {
    __wasm_call_ctors: () => unknown
    memory: WebAssembly.Memory
    meshopt_decodeFilterOct: Filter
    meshopt_decodeFilterQuat: Filter
    meshopt_decodeFilterExp: Filter
    meshopt_decodeIndexBuffer: Decoder
    meshopt_decodeIndexSequence: Decoder
    meshopt_decodeVertexBuffer: Decoder
    sbrk: (n: number) => number
  }
}
...
export const MeshoptDecoder = () => {
  ...
  const promise = WebAssembly.instantiate(unpack(wasm)).then((result) => {
    instance = result.instance as DecoderInstance
    instance.exports.__wasm_call_ctors()
  })

  function decode(fun: Decoder, target: Uint8Array, count: number, size: number, source: Uint8Array, filter?: Filter) {
  ...
  const getFilter = (filter: FilterKey): Filter | undefined =>
    filter !== 0 && filter !== 'NONE' ? instance.exports[filters[filter]] : undefined

  generated = {
    ready: promise,
    supported: true,
    decodeVertexBuffer(target, count, size, source, filter: FilterKey) {
      decode(
    ...
    decodeGltfBuffer(target, count, size, source, mode: DecoderMode, filter: FilterKey) {
      decode(instance.exports[decoders[mode]], target, count, size, source, getFilter(filter))
    },
  }
